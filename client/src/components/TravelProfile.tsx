...
const TravelProfileUserView = ({
  ...
  const { lang } = useLang();
  const { data, loading, error, refetch } = useQuery(GET_TRAVEL_PROFILE, {
    variables: { travelId },
    skip: !travelId || !isOpen,
    onError: (error) => {
      console.error("Error fetching Travel profile:", error.message);
    },
  });
        ...
        <div className="flex items-center justify-center bg-white h-full p-4">
          <div className="text-center max-w-md">
            ...
            <p className="text-gray-500 mb-6">
              {error.message ||
                "We couldn't retrieve this Travel's profile data. Please try again."}
            </p>
            <button
      ...
      return (
        <div className="flex items-center justify-center bg-white h-full p-4">
          <div className="text-center max-w-md">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-2">
              No Travel Profile Found
            </h2>
            Mail, Phone,
            <p className="text-gray-500 mb-6">
              We couldn't find a Travel profile with the provided ID. Please
              check the Travel ID and try again.
            </p>
          </div>
        </div>
      );
