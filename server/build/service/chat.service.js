...
class ChatService {
  ...
  async chatWithGuide(userId, guideId, message) {
    try {
      ...
      if (!room) throw HttpException_utils_1.default.notFound("room not found");
  ...
  async getChatByUserOfTravel(userId, travelId) {
    try {
      ...
      if (!chats) throw HttpException_utils_1.default.notFound;
      return chats;
    } catch (error) {
      if (error instanceof Error)
        throw HttpException_utils_1.default.badRequest(error.message);
  ...
  async readAllChatByUser(userId) {
    try {
      ...
      socket_1.io
        .to(userId)
        .emit("chat-count", { chatCount: chatCount.length });
      return updateChat;
    } catch (error) {
      if (error instanceof Error) {
        throw HttpException_utils_1.default.badRequest(error.message);
  ...
  async readChatByTravel(userId, travelId) {
    try {
      ...
      socket_1.io
        .to(userId)
        .emit("message-read-by-travel", { id: travelId, userId });
      return updateChat;
    } catch (error) {
      if (error instanceof Error) {
        throw HttpException_utils_1.default.badRequest(error.message);
