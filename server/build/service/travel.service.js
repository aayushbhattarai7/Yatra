...
class TravelService {
    ...
    userRepo;
    notificationRepo;
    constructor(travelrepo = database_config_1.AppDataSource.getRepository(travel_entity_1.Travel), travelDetailsrepo = database_config_1.AppDataSource.getRepository(travelDetails_entity_1.TravelDetails), locationRepo = database_config_1.AppDataSource.getRepository(location_entity_1.Location), travelRequestRepo = database_config_1.AppDataSource.getRepository(RequestTravels_entity_1.RequestTravel), travelKycRepo = database_config_1.AppDataSource.getRepository(travelKyc_entity_1.default), userRepo = database_config_1.AppDataSource.getRepository(user_entity_1.User), notificationRepo = database_config_1.AppDataSource.getRepository(notification_entity_1.Notification)) {
    ...
    async create(image, data) {
        return await database_config_1.AppDataSource.transaction(async (transactionalEntityManager) => {
            try {
                const emailExist = await transactionalEntityManager.findOne(this.travelrepo.target, {
                ...
                const travel = transactionalEntityManager.create(this.travelrepo.target, {
                    email: data.email,
                ...
                const traveldetails = transactionalEntityManager.create(this.travelDetailsrepo.target, {
                    DOB: data.DOB,
                ...
                await transactionalEntityManager.save(this.travelDetailsrepo.target, traveldetails);
                if (!travel) {
                    ...
                    if (image) {
                        ...
                        for (const key in image) {
                            ...
                            if (!allowedMimeTypes.includes(file.mimetype)) {
                                throw HttpException_utils_1.default.badRequest("Invalid image type. Only jpg, jpeg, and png are accepted.");
                            }
                            const kyc = transactionalEntityManager.create(this.travelKycRepo.target, {
                                name: file.name,
                            ...
                            const savedImage = await transactionalEntityManager.save(this.travelKycRepo.target, kyc);
                            savedImage.transferTravelKycToUpload(travel.id, savedImage.type);
                        }
                        if (travel) {
    ...
    async loginTravel(data) {
        try {
            ...
            const passwordMatched = await bcrypt_service_1.default.compare(data.password, travel.password);
            if (!passwordMatched) {
    ...
    async completeTravelService(travel_id, user_id) {
        try {
            ...
            return await database_config_1.AppDataSource.transaction(async (transactionEntityManager) => {
                ...
                const update = await transactionEntityManager.update(RequestTravels_entity_1.RequestTravel, { id: findTravelService.id }, {
    ...
    async sendPrice(price, travel_id, requestId) {
        try {
            ...
            if (data) {
                ...
                if (!notifications) {
                    throw HttpException_utils_1.default.notFound("Notifications not found");
                }
    ...
    async readNotification(travel_id) {
        try {
            ...
            const updateResult = await this.notificationRepo.update({ receiverTravel: { id: travel_id } }, { isRead: true });
            if (updateResult.affected && updateResult.affected > 0) {
                socket_1.io.to(travel_id).emit("notification-updated", { unreadCount: 0 });
            }
            return updateResult;
        }
        catch (error) {
    ...
    async rejectRequest(travel_id, requestId) {
        try {
            ...
            await this.travelRequestRepo.update({ id: request.id }, {
    ...
    async addLocation(travel_id, data) {
        try {
            ...
            if (isLocation) {
                ...
                socket_1.io.emit("travels", { location: travelLocation, id: travel_id });
                return message_1.Message.locationSent;
            }
        }
        catch (error) {
