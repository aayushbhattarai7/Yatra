...
class AdminService {
  ...
  travelRequestRepo;
  constructor(
    adminrepo = database_config_1.AppDataSource.getRepository(
  ...
  async login(data) {
    try {
      ...
      console.log(admin);
      if (!admin) throw HttpException_utils_1.default.notFound("Invalid Email");
      const checkPassword = await bcrypt_service_1.default.compare(
        data.password,
        admin.password,
      );
      if (!checkPassword)
      ...
      throw HttpException_utils_1.default.badRequest(
        error instanceof Error ? error.message : message_1.Message.error,
      );
    }
  }
  async getAdmin(id) {
  ...
  async approveTravel(adminId, travelId) {
    try {
      ...
      const data = await this.travelRepo.update(
        ...
        { approved: true, approval: enum_1.Status.ACCEPTED },
      );
  ...
  async rejectGuide(adminId, guideId, message) {
    try {
      ...
      const update = await this.guideRepo.update(
        { id: guideId },
        { approveStatus: message, approval: enum_1.Status.REJECTED },
      );
  ...
  async rejectTravel(adminId, travelId, message) {
    try {
      ...
      await mail_utils_1.default.sendMail(
        travel.email,
        "rejected",
        travel.approveStatus,
      );
