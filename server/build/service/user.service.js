...
const userImage_entity_1 = __importDefault(
  require("../entities/user/userImage.entity"),
);
...
class UserService {
  ...
  constructor(
    ...
    userImage = database_config_1.AppDataSource.getRepository(
      userImage_entity_1.default,
    ),
    ...
    guideRequestRepo = database_config_1.AppDataSource.getRepository(
    ...
    ratingsRepo = database_config_1.AppDataSource.getRepository(
      rating_entity_1.Rating,
    ),
    placeRepo = database_config_1.AppDataSource.getRepository(
    ...
    try {
      ...
      if (images) {
        ...
        if (images.cover) {
          ...
          savedCoverImage.transferImageToUpload(
            addUser.id,
            enum_1.MediaType.COVER,
          );
        }
      }
      ...
      if (images) {
        ...
        if (images.profile) {
          ...
          if (!profileImages) {
            ...
            (0, path_utils_1.transferImageFromUploadToTemp)(
              profileImages.id,
              profileImages.name,
              profileImages.type,
            );
            ...
            profileImages.transferImageToUpload(
              user.id,
              enum_1.MediaType.PROFILE,
            );
          }
  ...
  async reSendOtp(email) {
    try {
      ...
      if (!user)
        throw HttpException_utils_1.default.notFound(
          "Entered email is not registered yet",
        );
  ...
  async debugFBToken(userAccessToken) {
    if (
      !env_config_1.DotenvConfig.FACEBOOK_APP_ID ||
      !env_config_1.DotenvConfig.FACEBOOK_SECRET
    ) {
      throw HttpException_utils_1.default.internalServerError(
  ...
  async changePassword(password, confirmPassword, email) {
    try {
      ...
      if (password !== confirmPassword)
        throw HttpException_utils_1.default.badRequest(
          "passowrd must be same in both field",
        );
  ...
  async requestGuide(user_id, guide_id, data) {
    ...
    try {
      ...
      socket_1.io
        .to(guide_id)
        .emit("notification-count", unreadNotificationCount);
  ...
  async acceptTravelRequest(user_id, requestId) {
    try {
      ...
      if (data) {
        ...
        await mail_utils_1.default.sendAcceptedMail(
          user.email,
          "Travel",
          user.firstName,
        );
  ...
  async advancePaymentForTravel(userId, requestId, amount) {
    try {
      ...
      const stripe = new stripe_1.default(
        env_config_1.DotenvConfig.STRIPE_SECRET,
      );
    ...
    try {
      ...
      if (payment) {
        const data = await this.travelRequestRepo.update(
          ...
          {
            status: enum_1.RequestStatus.ACCEPTED,
            paymentType: enum_1.PaymentType.ESEWA,
            lastActionBy: enum_1.Role.USER,
          },
        );
        const room = await roomService.checkRoomWithTravel(
    ...
    try {
      ...
      if (payment) {
        await this.travelRequestRepo.update(
          ...
          {
            status: enum_1.RequestStatus.ACCEPTED,
            paymentType: enum_1.PaymentType.KHALTI,
            lastActionBy: enum_1.Role.USER,
          },
        );
        const notification = this.notificationRepo.create({
        ...
        socket_1.io
          .to(saveNotification.receiverTravel.id)
  ...
  async advancePaymentForGuideWithKhalti(userId, requestId, id) {
    try {
      ...
      if (payment) {
        ...
        socket_1.io
          .to(saveNotification.receiverGuide.id)
          .emit("notification", notifications);
        return (0, message_1.booked)("Guide");
      } else {
        throw HttpException_utils_1.default.badRequest("Payment unsuccessful");
