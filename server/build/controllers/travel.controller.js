...
class TravelController {
    async create(req, res) {
        try {
            ...
            const details = await travel_service_1.default.create(uploadedPhotos, req.body);
    ...
    async verifyToken(req, res) {
        try {
            ...
            const token = await webToken_service_1.default.verify(refreshToken, env_config_1.DotenvConfig.ACCESS_TOKEN_SECRET);
            if (!token) {
                throw HttpException_utils_1.default.unauthorized("You are not authorized");
            }
            const newAccessToken = webToken_service_1.default.generateAccessToken(token, token.role);
    ...
    async sendPrice(req, res) {
        try {
            ...
            const data = await travel_service_1.default.sendPrice(price, travel_id, requestId);
            res.status(StatusCodes_1.StatusCodes.SUCCESS).json({ data });
        }
        catch (error) {
            if (error instanceof Error) {
    ...
    async acceptRequest(req, res) {
        try {
            ...
            const data = await travel_service_1.default.acceptRequest(travel_id, requestId);
            res
    ...
    async rejectRequest(req, res) {
        try {
            ...
            res.status(StatusCodes_1.StatusCodes.SUCCESS).json({ message: "Request rejected" });
        }
        catch (error) {
            if (error instanceof Error) {
                res.status(StatusCodes_1.StatusCodes.BAD_REQUEST).json({
    ...
    async addLocation(req, res) {
        try {
            const travel_id = req.user?.id;
            const data = await travel_service_1.default.addLocation(travel_id, req.body);
            res.status(StatusCodes_1.StatusCodes.SUCCESS).json({ data });
        }
        catch (error) {
            if (error instanceof Error) {
                res.status(StatusCodes_1.StatusCodes.BAD_REQUEST).json({ message: error.message });
            }
            else {
                res
