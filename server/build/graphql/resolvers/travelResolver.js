...
const travel_service_1 = __importDefault(
  require("../../service/travel.service"),
);
...
class TravelResolver {
  ...
  async getUnreadNotificationsOfTravel(ctx) {
    try {
      const travelId = ctx.req.user?.id;
      return await travel_service_1.default.getUnreadNotificationsCount(
        travelId,
      );
    } catch (error) {
      if (error instanceof Error) {
  ...
  async requestForCompletedTravel(userId, ctx) {
    try {
      const travelId = ctx.req.user?.id;
      return await travel_service_1.default.completeTravelService(
        travelId,
        userId,
      );
    } catch (error) {
...
__decorate(
  [
    ...
    (0, type_graphql_1.UseMiddleware)(
      ...
      (0, authorization_middleware_1.authorization)([enum_1.Role.TRAVEL]),
    ),
