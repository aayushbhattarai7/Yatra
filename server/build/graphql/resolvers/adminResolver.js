...
let AdminResolver = class AdminResolver {
  ...
  async rejectTravel(ctx, travelId, message) {
    try {
      const adminId = ctx.req.user?.id;
      return await admin_service_1.default.rejectTravel(
        adminId,
        travelId,
        message,
      );
  ...
  async rejectGuide(ctx, guideId, message) {
    try {
      const adminId = ctx.req.user?.id;
      return await admin_service_1.default.rejectGuide(
        adminId,
        guideId,
        message,
      );
...
__decorate(
  [
    ...
    (0, type_graphql_1.UseMiddleware)(
      ...
      (0, authorization_middleware_1.authorization)([enum_1.Role.ADMIN]),
    ),
