...
let UserResolver = class UserResolver {
  ...
  async changePasswordOfUser(email, password, confirmPassword) {
    try {
      return await user_service_1.default.changePassword(
        password,
        confirmPassword,
        email,
      );
  ...
  async updatePasswordOfUser(currentPassword, password, confirmPassword, ctx) {
    try {
      ...
      return await user_service_1.default.updatePassword(
        id,
        password,
        confirmPassword,
        currentPassword,
  ...
  async completeTravelServiceByUser(travelId, ctx) {
    try {
      const userId = ctx.req.user?.id;
      return await user_service_1.default.completeTravelService(
        userId,
        travelId,
      );
    } catch (error) {
  ...
  async rateTravel(id, rating, message, ctx) {
    try {
      ...
      return await user_service_1.default.rateTravel(
        userId,
        id,
        rating,
        message,
  ...
  async rateGuide(id, rating, message, ctx) {
    try {
      const userId = ctx.req.user?.id;
      return await user_service_1.default.rateGuide(
        userId,
        id,
        rating,
        message,
  ...
  async getTravelLocation(ctx, travel_id) {
    try {
      ...
      const data = await user_service_1.default.getTravelLocation(
        user_id,
        travel_id,
      );
      return data;
  ...
  async getGuideLocation(ctx, guide_id) {
    try {
      const user_id = ctx.req.user?.id;
      const data = await user_service_1.default.getGuideLocation(
        user_id,
        guide_id,
      );
      return data;
  ...
  async sendPriceToGuide(requestId, price, ctx) {
    try {
      const userId = ctx.req.user?.id;
      return await user_service_1.default.sendGuidePrice(
        price,
        userId,
        requestId,
      );
  ...
  async sendPriceToTravel(requestId, price, ctx) {
    try {
      const userId = ctx.req.user?.id;
      return await user_service_1.default.sendTravelPrice(
        price,
        userId,
        requestId,
      );
  ...
  async cancelTravelRequest(requestId, ctx) {
    try {
      const userId = ctx.req.user?.id;
      return await user_service_1.default.cancelTravelRequest(
        userId,
        requestId,
      );
    } catch (error) {
  ...
  async AdvancePaymentForTravel(travelId, amount, ctx) {
    try {
      const userId = ctx.req.user?.id;
      return await user_service_1.default.advancePaymentForTravel(
        userId,
        travelId,
        amount,
      );
  ...
  async AdvancePaymentForGuide(guideId, amount, ctx) {
    try {
      const userId = ctx.req.user?.id;
      return await user_service_1.default.advancePaymentForGuide(
        userId,
        guideId,
        amount,
      );
  ...
  async removeFromFavourite(placeId, ctx) {
    try {
      const userId = ctx.req.user?.id;
      return await place_service_1.default.removePlaceToFavourite(
        userId,
        placeId,
      );
    } catch (error) {
...
__decorate(
  [
    ...
    (0, type_graphql_1.UseMiddleware)(
      authentication_middleware_1.authentication,
      (0, authorization_middleware_1.authorization)([enum_1.Role.USER]),
    ),
    __param(0, (0, type_graphql_1.Arg)("email")),
